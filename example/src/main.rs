...
use richamster_api::richamster::common::Richamster;
...
async fn main() -> Result<(), Error> {
    ...
    loop {
        ...
        match item {
            ...
            MenuItems::UserDetails => match show_user_details(&token_storage).await {
                Ok(_) => {}
                Err(e) => {
                    eprintln!("Error showing user details: {:?}", e);
                }
            },
            ...
            MenuItems::TickerList => {
                let _ = show_ticker_list().await;
            }
            MenuItems::RestrictionsList => {
                let _ = show_restrictions_list().await;
            }
...
async fn show_user_details(token_storage: &JwtTokenStorage) -> Result<(), RichamsterError> {
    ...
    println!("token: {:?}", token_storage.token.clone().unwrap());
...
async fn show_ticker_list() -> Result<(), RichamsterError> {
    let exchange: Exchange = Exchange::new();
    let result = exchange.ticker_list(None).await?;
    println!("{:?}", result);
    Ok(())
}

async fn show_restrictions_list() -> Result<(), RichamsterError> {
    let exchange = Exchange::new();
    let result = exchange.restrictions_list().await?;
    println!("{:?}", result);
    Ok(())
}
